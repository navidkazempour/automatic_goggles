		...
		twitter.get('search/tweets', {q: searchTerm, lang: "en", result_type: "popular", count: 12}, function(error, tweets, response) {
				if (error) {
					...
					for(var i =0 ;i < tweets.statuses.length;i++){
						var url = "https://api.twitter.com/1/statuses/oembed.json?url=https://twitter.com/"+tweets.statuses[i].user.screen_name+"/status/"+tweets.statuses[i].id_str;						request(url,function(err,res,body){
							if(err){
								console.log(err);
							}else{
